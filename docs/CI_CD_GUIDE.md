...
The 40 Java repositories are organized into 7 dependency layers. Repos in the same layer can be built/released in parallel because they do not depend on each other. Repos in layer N only depend on repos in layers 0 through N-1.
...
GitHub Packages does **not** allow overwriting an existing version. Once `26.02.07` is published for a given artifact, that version is permanent. Attempting to re-publish returns HTTP 409 Conflict.
...
- This immutability is a good thing: it guarantees that version `26.02.07` always refers to the same bytes, everywhere
...
#    ... and so on until all 40 Java repos are released

# 5. Release non-Java repos independently (they have no layer dependencies)
cd fireflyframework-cli && git push origin main v26.02.07
cd fireflyframework-genai && git push origin main v26.02.07
...
# 2. Layer 0: parent
cd fireflyframework-parent && git push origin main v26.02.07
...
for repo in fireflyframework-bom fireflyframework-utils fireflyframework-cache ...; do
  cd "$repo" && git push origin main v26.02.07 && cd ..
done
...
**What you see:** `Could not find artifact org.fireflyframework:fireflyframework-xyz:jar:26.02.07`
